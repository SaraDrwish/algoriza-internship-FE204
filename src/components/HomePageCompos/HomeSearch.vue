  ...
  <div class="">
    <div class="relative w-[calc(100%-105px)] left-[7%] mx-auto h-[64px]">
      <form
        @submit.prevent
        action="#"
        class="flex rounded-[8px] gap-[15px] shadow-lg bg-white pt-[10px] pb-[11px] pr-[13px] pl-[12px]"
      >
        <div class="bigsearchdropdnbox relative w-[286px] text-[13px]">
          <div class="flex bg-inputsGray rounded-[4px]">
            <div
              class="flex items-center gap-[10px] mx-[12px] mt-[11px] mb-[12px]"
            >
              <img
                class="flex"
          ...
          <div
            v-if="modalActive"
            class="absolute top-[70px] z-10 flex bg-inputsGray w-full rounded-[10px]"
          >
            <ul
              v-if="dropdownOptions.length > 0"
              class="flex items-center capitalize transition ease-in duration-400 text-light-black w-full justify-center text-center flex-col gap-[4px] mx-[12px]"
            >
              <a
                v-for="searchresult in dropdownOptions"
                :key="searchresult.id"
                @click="selectCountry(searchresult)"
                class="hover:transition-all transition ease-in duration-400 hover:text-primary w-[100%] py-[10px] border-solid border-b-2 border-bordrBtnGry cursor-pointer"
                href="#"
              >
        ...
        <div
          class="flex gap-[10px] bg-inputsGray rounded-[4px] pt-[11px] pb-[12px] px-[12px]"
        >
        ...
        >
          <img
            src="../../assets/icons/single_bed_FILL0_wght400_GRAD0_opsz24 1.svg"
            alt="single_bed"
          />
          <input
            v-model="rooms"
        ...
        <input
          @click="handleSubmit"
          class="flex tracking-[0.3px] w-[137px] justify-center bg-primary text-[15px] text-white cursor-pointer py-[12px] px-[18px] rounded-[6px]"
          type="submit"
          value="Search"
        />
      </form>
...
const validateForm = () => {
  ...
  if (checkOutDate.value < minCheckOutDate.toISOString().split("T")[0]) {
    console.error(
      "Check-out date cannot be before a day from the check-in date"
    );
